...
class ClockView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        var date = Date()
        setTime(date.day,date.minutes,date.minutes)
        // todo 1: 每一秒刷新一次，让指针动起来
    }

    private var mSecondDegree : Float  = 0f
    private var mMinDegree: Float  = 0f
    private var mHourDegree : Float  = 0f
    private var mIsNight = false

    fun setTime(hour: Int, min: Int, second: Int) {
        if (hour >= 24 || hour < 0 || min >= 60 || min < 0 || second >= 60 || second < 0) {
            Toast.makeText(context, "时间不合法", Toast.LENGTH_SHORT).show()
            return
        }
        if (hour >= 12) {
            mIsNight = true
    ...
    private fun drawNeedles(canvas: Canvas) {
        ...
        // 画分针
        // todo 2: 画分针
        val part2 = nowSeconds / 12
        drawPointer(canvas, POINTER_TYPE_MINUTES, 5 * nowMinutes +part2)
        // 画时针
